class SiteController < ApplicationController
  ...
  def homepage
     ...
     @pagy, @posts = pagy(Post.where.not(customer_id: nil).where.not(publication_date: nil).where(active: true).where("publication_date <= ?", Date.today).order(publication_date: :desc), limit: 6)
  end
end
